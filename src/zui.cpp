...
void drawTextBaseline(QPainter *p, int x, int basey, bool hcenter, QRect clip, QString str)
{
    ...
    if (!clip.isEmpty())
    {
        p->setClipRect(clip);
        if (hcenter)
            x = clip.left() + (clip.width() - fm.horizontalAdvance(str)/*.width()*/) / 2;
    }
...
void drawTextBaseline(QPainter *p, QPointF pos, bool hcenter, QRectF clip, QString str)
{
    ...
    if (!clip.isEmpty())
    {
        p->setClipRect(clip);
        if (hcenter)
            pos.setX(clip.left() + (clip.width() - fm.horizontalAdvance(str)/*.width()*/) / 2);
    }
    p->drawText(pos, str);
...
void restrictWidgetWiderSize(QWidget *widget, double charnum, AdjustedValue val)
{
    ...
    int w = _sizeHelper(charnum, mmax(fm.averageCharWidth(), fm.horizontalAdvance('W'), fm.horizontalAdvance('M'), fm.horizontalAdvance('X')));
    if (val == AdjustedValue::Min || val == AdjustedValue::MinMax)
        widget->setMinimumWidth(w);
    if (val == AdjustedValue::Max || val == AdjustedValue::MinMax)
        widget->setMaximumWidth(w);
...
{
    QFontMetrics fm = widget->fontMetrics();
    return _sizeHelper(charnum, mmax(fm.averageCharWidth(), fm.horizontalAdvance('W'), fm.horizontalAdvance('M'), fm.horizontalAdvance('X')));
}
...
QString formatDate(const QDate &d, QString unset)
{
    if (!d.isValid())
        return unset;
    return DateTimeFunctions::format(d.startOfDay(), DateTimeFunctions::NormalDate);
}
