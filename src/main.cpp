...
int main(int argc, char **argv)
{
    ...
    if (args.contains("--help") || args.contains("-h") || args.contains("-?") || args.contains("/help") || args.contains("/h") || args.contains("/?"))
    {
        ...
        out << "zkanji " << ZKANJI_PROGRAM_VERSION << Qt::endl;
        out << "USAGE: zkanji [option]" << Qt::endl;
        out << Qt::endl;
        out << "  --help, -h, -?, /help, /h, /?" << Qt::endl;
        out << "                  show command line flags without starting the program." << Qt::endl;
        out << Qt::endl;
        out << "  -i [path]       import JMdict dictionary data at startup from files at path." << Qt::endl;
        out << Qt::endl;
        out << "  Files needed for import are: JMdict in UTF-8 encoding," << Qt::endl;
        out << "                               kanjidic in EUC-JP encoding," << Qt::endl;
        out << "                               radkfile in EUC-JP encoding," << Qt::endl;
        out << "                               kanjiorder.txt," << Qt::endl;
        out << "                               JLPTNData.txt," << Qt::endl;
        out << "                               radkelement.txt," << Qt::endl;
        out << "                               zradfile.txt" << Qt::endl;
        out << Qt::endl;
        out << "  -e [path]       import the Tanaka Corpus example sentences data at startup" << Qt::endl;
        out << "                  from files at path. Only works if the dictionary data is" << Qt::endl;
        out << "                  already generated." << Qt::endl;
        out << Qt::endl;
        out << "  Files needed for import are: examples.utf the Tanaka Corpus in UTF-8" << Qt::endl;
        out << "                               encoding." << Qt::endl;
        out << Qt::endl;
        out << "  -ie [path]      can be used when the files are located at the same path." << Qt::endl;
        out.flush();
        exit(0);
    }
