    ...
    if (e->type() == ScrollAreaScrolledEvent::Type())
    {
        ...
        if (underMouse())
        {
            if (se->wheelEvent() != nullptr)
                qApp->postEvent(this, new QMouseEvent(QEvent::MouseMove, se->wheelEvent()->position(), Qt::NoButton, se->wheelEvent()->buttons(), se->wheelEvent()->modifiers()));
            else
            {
...
{
    if (!QRectF(drawArea()).contains(e->position()) && (type != Scrollbar || !QRectF(scrollArea()).contains(e->position())))
        return;
