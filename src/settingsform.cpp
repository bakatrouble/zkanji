...
QSize fontPreviewWidget::sizeHint() const
{
    ...
    if (sizeheight != -1)
        return {m.left() + m.right() + 1, m.top() + m.bottom() + sizeheight};
...
void fontPreviewWidget::paintEvent(QPaintEvent *e)
{
    ...
    r.setLeft(r.left() + nfm.horizontalAdvance(str));
    p.setFont(mf);
...
void SettingsForm::on_kanjiSizeCBox_currentIndexChanged(int /*index*/)
{
    ...
    QMargins m = ui->kanjiPreview->contentsMargins();
    ui->kanjiPreview->setFixedSize(cellsize + m.left() + m.right(), cellsize + m.top() + m.bottom());
