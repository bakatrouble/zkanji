...
void ZRadicalGrid::recomputeItems()
{
    ...
    int radcommaw = radmet.horizontalAdvance(comma);
    int namecommaw = namemet.horizontalAdvance(comma);
    int radcnt = tosigned(ZKanji::radlist.size());
    ...
    for (int ix = 0, siz = tosigned(list.size()); ix != siz; ++ix)
    {
        ...
        if (names)
        {
            ...
            int upperwidth = namemet.horizontalAdvance(dat->names[upperpos].toQString());
            int lowerwidth = namemet.horizontalAdvance(dat->names[lowerpos].toQString());
            while (upperpos + 1 < lowerpos)
            {
                if (upperwidth <= lowerwidth)
                {
                    ...
                    ++upperpos;
                    upperwidth += namemet.horizontalAdvance(dat->names[upperpos].toQString());
                }
                else
                {
                    lowerwidth += namecommaw;
                    --lowerpos;
                    lowerwidth += namemet.horizontalAdvance(dat->names[lowerpos].toQString());
                }
            }
            upperwidth += namecommaw;
