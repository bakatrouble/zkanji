...
void KanjiInfoForm::saveState(KanjiInfoData &dat) const
{
    ...
    dat.pos = g.topLeft();
    dat.screen = qApp->screens().at(screenNumber(QRect(dat.pos, dat.siz)))->geometry();
...
void KanjiInfoForm::restoreState(const KanjiInfoData &dat)
{
    ...
    {
        QRect sg = qApp->screens().at(screenNumber(QRect(dat.pos, dat.siz)))->geometry();
        if (dat.screen == sg)
    ...
    else if (Settings::kanji.showpos == KanjiSettings::NearCursor)
    {
        ...
        QRect sg = qApp->screenAt(p)->geometry();
