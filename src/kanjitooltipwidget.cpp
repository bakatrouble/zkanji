...
void KanjiToolTipWidget::paintEvent(QPaintEvent *e)
{
    ...
    int spacew = fm.horizontalAdvance(QChar(' '));
    ...
    rowx += kfm.horizontalAdvance(k->ch)/*.width()*/ + hpad;
...
void KanjiToolTipWidget::recalculateSize()
{
    ...
    int minwidth = kfm.horizontalAdvance(k->ch)/*.width()*/ + hpad + penw + hpad + fbm.boundingRect(tr("Strokes:")).width() + hpad * 3 + fxlm.boundingRect(QString::number((int)k->strokes)).width() + hpad + penw + hpad + fbm.boundingRect(tr("Radical:")).width() + hpad * 3 + rfm.boundingRect(ZKanji::radsymbols[k->rad - 1]).width() + hpad * 4;
    ...
    for (int ix = 0, siz = k->on.size(); ix != siz; ++ix)
    {
        int wordw = kkfm.horizontalAdvance(k->on[ix].toQString() % (ix == siz - 1 ? QChar() : QChar(',')));
    ...
    for (int ix = 0, siz = k->kun.size(); ix != siz; ++ix)
    {
        int wordw = kkfm.horizontalAdvance(k->kun[ix].toQString() % (ix == siz - 1 ? QChar() : QChar(',')));
        if (linew == rleft || linew + spacew + wordw < maxwidth)
            linew += (linew == rleft ? 0 : spacew) + wordw;
        else
        {
