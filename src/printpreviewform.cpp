...
void PrintTextBlock::setFuriWord(WordEntry *e, QFont &f, QFontMetrics &fm, QFont &ff, QFontMetrics &ffm, int lineheight, int descent, int furiheight, int furidescent)
{
    ...
    w = fm.horizontalAdvance(e->kanji.toQStringRaw());
    if (w <= maxwidth)
    {
    ...
    else
    {
        ...
        for (int pos = 0, siz = tosigned(e->kanji.size()); pos != siz; ++pos)
        {
            ...
            {
                strw = fm.horizontalAdvance(word->kanji.toQString(fdat[datpos].kanji.pos, fdat[datpos].kanji.len));
                datfit = true;
            }
            ...
            {
                strw = fm.horizontalAdvance(e->kanji[pos]);
                datfit = false;
            }
...
void PrintTextBlock::addString(int tokenpos, QFontMetrics &fm, bool addspace)
{
    ...
    int dw = left == 0 || delim.isEmpty() ? 0 : fm.horizontalAdvance(delim);
    ...
    for (int ix = 0; ix != str.size() + 1; ++ix)
    {
        ...
        int chw = !finish ? fm.horizontalAdvance(str.at(ix)) : 0;
        ...
        for (int ix = 0, siz = tosigned(list.size()); ix != siz; ++ix)
        {
            ...
            if (word != nullptr && ((frontword && list[ix].tokenpos == 0) || (!frontword && list[ix].tokenpos == tosigned(tokens.size()) - 1)))
            {
                ...
                while (datpos < tosigned(fdat.size()) && (list[ix].pos == -1 || (int)fdat[datpos].kanji.pos < list[ix].pos + list[ix].len))
                {
                    ...
                    {
                        leftpos += setlist[setpos].fm.horizontalAdvance(str.mid(strpos, fdat[datpos].kanji.pos - strpos));
                    ...
                    {
                        int strw = setlist[setpos].fm.horizontalAdvance(str.mid(fdat[datpos].kanji.pos, fdat[datpos].kanji.len));
                    ...
                    int strw = setlist[setpos].fm.horizontalAdvance(strpart);
...
void PrintTextBlock::paintKanjiFuri(QPainter &p, int x, int y, bool rightalign)
{
    ...
    for (int pos = 0; pos != kanjisiz; ++pos)
    {
        ...
        if (datpos == fdat.size() || (int)fdat[datpos].kanji.pos > pos)
        {
            ...
            leftpos += fm.horizontalAdvance(word->kanji[pos]);
            continue;
        }
...
void PrintTextBlock::paintFurigana(QPainter &p, int x, int y, int width, int pos, int len)
{
    ...
    int kanaw = furifm.horizontalAdvance(str);
    ...
    for (int ix = 0; ix != len; ++ix)
    {
        ...
        if (ix != len - 1)
        {
            ...
            x += furifm.horizontalAdvance(str.at(ix)) + chspace;
...
void PrintPreviewForm::paintPages(QPrinter *pr)
{
    ...
    QRect pagerect = pr->pageLayout().paintRectPixels(pr->resolution());
    ...
    int spacewidth = dfm.horizontalAdvance(' ');
